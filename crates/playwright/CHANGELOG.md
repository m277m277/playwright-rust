...
- **Bumped `crossbeam-epoch` 0.9.18 → 0.9.20 for [RUSTSEC-2026-0204](https://rustsec.org/advisories/RUSTSEC-2026-0204)** (invalid pointer dereference in a `fmt::Pointer` impl). The crate reaches this repo only through `criterion`, a dev-dependency used for benchmarks, so it is not present in the dependency tree of anything that depends on `playwright-rs` — no consumer was exposed. The lockfile bump keeps `cargo audit` / `cargo deny` green.
